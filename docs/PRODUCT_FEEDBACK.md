...
| Starlette / Uvicorn | The existing MCP application serves the API and static browser app. | One service prevents a disconnected demonstration backend. | Authentication, origin checks and TLS still require application controls; the test stack currently emits a non-fatal AnyIO compatibility deprecation. | Yes, with the boundary tests retained and the upstream compatibility warning monitored. |
...
| GitHub plugin | Repository reads and Git-object/PR operations support cloud-first work. | Source and final state can be reconciled without depending on one computer. | A response error may leave the mutation outcome uncertain (F-007); read back before acting again. | Yes, with idempotent reconciliation. |
...
| Bandit | The release job scans Python source and saves a machine-readable report. | It surfaced controlled subprocess and temporary-path patterns that were then narrowed and documented. | A clean report does not prove the absence of logic or authorization flaws; justified suppressions must stay exact. | Yes, as a blocking source-security gate beside tests and threat-model review. |
...
| YouTube Studio | One 1:28 narrated recording was published with English captions. | Complete Brave playback reached the end with presentation, captions and sound working. | Browser mutation responses can time out; saved metadata needs an independent readback. Text URLs need channel verification to be clickable. | Yes, keeping the current public upload stable through judging. |
...
it is not Alexa, a cloned voice or the entrant's recorded voice.
