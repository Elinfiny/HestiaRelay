...
| Git / GitHub | Public MIT source, feature branches and reviewed PRs preserve history. | Exact commit identities tie results to implementation. | Initial repository creation required a manual step (F-001); current main has no provider-enforced branch protection. | Yes, retaining guarded PR merges and public evidence. |
